...
module Beaneater
  ...
  class Connection
    ...
    def parse_response(cmd, res)
      ...
      raise UnexpectedResponse.from_status(status, cmd) if UnexpectedResponse::ERROR_STATES.include?(status) if ["TIMED_OUT", "NOT_FOUND"].include?(status)

      raise RuntimeError, "Unknown command. status from server: #{status}, sent cmd: #{cmd}, result was: #{res}" if UnexpectedResponse::ERROR_STATES.include?(status)
      body = nil
      if ['OK','FOUND', 'RESERVED'].include?(status)
        ...
        raise RuntimeError, "Error: status from server: #{status}, sent cmd: #{cmd}, crlf: #{crlf} result was: #{res} Expected crlf" if crlf != "\r\n"
      end
      id = body_values[1]
